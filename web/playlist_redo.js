...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== 'SkebaPlaylistRedoSave') return;
    ...
    async nodeCreated(node) {
        if (node.comfyClass !== 'SkebaPlaylistRedoSave') return;
        node.addWidget('button', 'Register redo workflow', null, async () => {
            const name = window.prompt('Name this two-pass redo workflow', 'H3 two-pass redo');
            if (name === null) return;
            try {
                const { output } = await app.graphToPrompt();
                const response = await api.fetchApi('/api/h3-video-playlist/templates', {
                    method: 'POST', headers: {'Content-Type':'application/json'},
                    body: JSON.stringify({name, graph:output}),
                });
