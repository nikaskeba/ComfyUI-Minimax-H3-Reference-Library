...
AV_MASK_PATCH_MARKER = "_h3_existing_video_av_mask_payload_compat_v2"
...
def _already_patched(cls):
    ...
    if getattr(fn, "__name__", "") == "_patched_extra_conds":
        return "other"
    # H3-Multishot's AV-bank wrapper performs the same keyframe-first video
    # merge and preserves reference audio/frame_count. Older installed builds
    # expose only their own marker, so recognize it by both marker and module.
    where = str(getattr(fn, "__module__", "") or "").replace("\\", "/")
    if (getattr(fn, MULTISHOT_PATCH_MARKER, False)
            and (where == "h3_avbank_probe"
                 or where.endswith(".h3_avbank_probe")
                 or where.endswith("/h3_avbank_probe"))):
        return "h3_multishot"
    # MultiRef's AV-mask compatibility wrapper is transparent: it calls the
    # wrapped payload builder first, then only adds denoise-mask conditions.
    # Accept it when its __wrapped__ chain still contains a proven compatible
    # Motion Context payload merge. Do not generalize this to arbitrary
    # wrappers; an unknown outer owner may rewrite the same latent lists.
    if getattr(fn, AV_MASK_PATCH_MARKER, False):
        wrapped = getattr(fn, "__wrapped__", None)
        seen = set()
        while wrapped is not None and id(wrapped) not in seen:
            seen.add(id(wrapped))
            if getattr(wrapped, PATCH_MARKER, False):
                return "same_under_av_mask"
...
def apply_patch():
    ...
    if who:
        ...
        elif who == "same_under_av_mask":
            _LOG.info("h3_motion_context: compatible keyframe/ref merge is "
                      "active under MultiRef AV-mask compatibility; standing "
                      "down")
        elif who == "h3_multishot":
            _LOG.info("h3_motion_context: H3-Multishot's compatible AV-bank "
                      "payload merge is active; standing down")
        else:
            _LOG.warning(
