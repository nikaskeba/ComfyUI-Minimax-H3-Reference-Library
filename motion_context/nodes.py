...
_LOG = logging.getLogger("skeba_h3_motion_context")
...
class MiniMaxH3MotionContext:
        ...
        return {
            ...
            "optional": {
                ...
                "bypass": ("BOOLEAN", {
                    ...
                    "label_on": "BYPASSED",
                    "label_off": "ENABLED",
                    "tooltip": "Pass conditioning through unchanged and "
    ...
    def apply(self, conditioning, vae, latent, context_length,
        ...
        if (isinstance(context_latent, dict)
            ...
            raise ValueError(
                "h3_motion_context: Load Latent is bypassed while Motion "
        ...
        if context_latent is not None:
            ...
            if src_w != width or src_h != height:
                ...
                raise ValueError(
                    ...
                    "resolution, or the chain restarted here. This "
                    "invocation received bypass=False (ENABLED)."
...
class MiniMaxH3MotionContextLoadLatent:
        ...
        return {
            ...
            "optional": {
                "bypass": ("BOOLEAN", {
                    ...
                    "label_on": "BYPASSED",
                    "label_off": "ENABLED",
                    "tooltip": "Do not access the latent file. Turn this on "
                               "for the first clip and drive it from the same "
                               "boolean as Motion Context and Trim."}),
            },
        }
    ...
    @classmethod
    def IS_CHANGED(cls, latent_path, clip_index=0, bypass=False):
    ...
    def load(self, latent_path, clip_index=0, bypass=False):
        if bypass:
            _LOG.info("h3_motion_context: Load Latent bypassed")
            # This typed placeholder lets the graph execute without a file on
            # the first pass. Motion Context returns before inspecting it when
            # both nodes share the same bypass boolean.
            return ({"samples": None,
                     "skeba_h3_motion_context_bypassed": True},)
...
NODE_CLASS_MAPPINGS = {
    ...
    "SKEBAMiniMaxH3MotionContextLoadLatent": MiniMaxH3MotionContextLoadLatent,
