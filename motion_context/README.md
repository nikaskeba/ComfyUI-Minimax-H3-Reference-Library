...
This is a local fork of H3 Motion Context. Its node IDs and display names are
separate from the upstream pack, and it adds shared boolean bypass switches for
automated first-pass/continuation workflows.
...
For an automated two-pass workflow, the **Motion Context**, **Trim**, and
**Load Latent** nodes each have a `bypass` boolean. Drive all three from the
same boolean: `true` on the first pass, when no previous latent exists, and
`false` on the second and later passes. While bypassed, Motion Context passes
conditioning through and reports zero trim frames, Trim passes picture and
sound through unchanged, and Load Latent does not touch the filesystem.

The switch reads `BYPASSED` when true and `ENABLED` when false. A list such as
`[true, false, false]` bypasses only its first mapped item; the remaining items
run Motion Context normally. ComfyUI list mapping is not a sequential clip
loop: every mapped Motion Context call runs before a downstream Save Latent can
write the preceding item's file. Queue chained clips as separate executions, or
use a loop mechanism that explicitly completes each sampler/save before starting
the next clip.

For automated loops, **H3 Seam Exposure Match** can sit after the final image
upscaler and before Create Video. Connect the loop's incoming accumulation so
the node can compare the previous clip's last six frames with the new clip's
first six. The default strength of `0.75` fades a luminance-only correction back
to neutral over 24 frames. Set strength to `0` to disable it. This is intended
for continuous shots; leave it off when a hard lighting change is deliberate.
...
0 to disable the overlap output. This affects picture onlyâ€”Trim's main audio
...
python tests/_seam_exposure_test.py  # brightness clamp, fade and pass-through
...
The first four print their checks and end with a pass line. The other three
...
| `nodes.py` | Motion Context, Trim, and the latent Save/Load pair. |
| `seam_exposure.py` | Conservative luminance matching for decoded clip joins. |
