...
def make_mm(ref_advance_factor=1.0, audio_rows_per_step=2, modern=False):
    ...
    if modern:
        legacy_init = PackedLayout.__init__

        def modern_init(self, text_len, latent_t, latent_h, latent_w,
                        audio_t, keyframes=None, refs=None):
            frame_count = sum(FRAME_PER_TOKEN[k % 5]
                              for k in range(latent_t))
            legacy_init(self, text_len, latent_t, latent_h, latent_w,
                        audio_t, keyframes=keyframes, refs=refs,
                        frame_count=frame_count)
            origin = float(self.position_ids[self.segments[-1][0], 0])
            cond = [(a, b) for a, b, kind in self.segments
                    if kind == "cond"]
            for (a, b), kf in zip(cond, keyframes or []):
                self.position_ids[a:b, 0] = (
                    origin + FRAME_RESCALE * kf["resolved_frame_index"])
...
def main():
    ...
    modern = make_mm(modern=True)
    modern_patch = load_patch(modern)
    assert modern_patch.apply_patch(), \
        "self-test failed against current no-frame_count layout"
    modern_run = [{"resolved_frame_index": 0,
                   modern_patch.MC_KEY: i} for i in range(4)]
    modern_lay = modern.PackedLayout(
        TEXT_LEN, LATENT_T, LH, LW, AUDIO_T, keyframes=modern_run)
    assert np.allclose(_cond_ts(modern_lay),
                       [TEXT_LEN + FRAME_RESCALE * i for i in range(4)])
    print("1b. current no-frame_count constructor passes and places anchors")
