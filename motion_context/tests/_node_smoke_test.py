...
def main():
    ...
    assert set(pkg.NODE_CLASS_MAPPINGS) == {
        "SKEBAMiniMaxH3MotionContext",
        "SKEBAMiniMaxH3MotionContextTrim",
    ...
    # All three runtime nodes expose a linkable boolean switch. On the first
    # pass they must do no context work, no trimming, and no filesystem read.
    for cls in (nodes.MiniMaxH3MotionContext,
                nodes.MiniMaxH3MotionContextTrim,
                nodes.MiniMaxH3MotionContextLoadLatent):
        bypass_spec = cls.INPUT_TYPES()["optional"]["bypass"]
        assert bypass_spec[0] == "BOOLEAN"
        assert bypass_spec[1]["default"] is False
        assert bypass_spec[1]["label_on"] == "BYPASSED"
        assert bypass_spec[1]["label_off"] == "ENABLED"

    untouched = [["c", {"existing": True}]]
    bypassed_cond, bypassed_trim = nodes.MiniMaxH3MotionContext().apply(
        conditioning=untouched, vae=None, latent=None, context_length="22",
        bypass=True)
    assert bypassed_cond is untouched and bypassed_trim == 0
    assert not nodes._layout_patch_applied(), \
        "bypassed Motion Context installed the layout patch"
    ...
    assert placeholder["samples"] is None
    assert placeholder["skeba_h3_motion_context_bypassed"] is True
    assert loader.IS_CHANGED("path/that/does/not/exist", bypass=True) == "bypassed"
    try:
        nodes.MiniMaxH3MotionContext().apply(
            conditioning=untouched, vae=None, latent=None,
            context_length="22", context_latent=placeholder)
    except ValueError as e:
        assert "Load Latent is bypassed" in str(e), str(e)
    else:
        raise AssertionError("enabled Motion Context accepted bypassed loader")
    print("bypass switches: pass-through/no-read behavior verified")
