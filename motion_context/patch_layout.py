...
_orig_init = None
_orig_accepts_frame_count = False
_applied = False
...
def _cond_t(text_len, latent_t, frame_count, p):
    ...
    if (_orig_accepts_frame_count and frame_count is not None
            and p == frame_count - 1):
...
def _call_orig_init(self, text_len, latent_t, latent_h, latent_w, audio_t,
                    keyframes=None, refs=None, frame_count=None):
    kwargs = {"keyframes": keyframes, "refs": refs}
    if _orig_accepts_frame_count:
        kwargs["frame_count"] = frame_count
    _orig_init(self, text_len, latent_t, latent_h, latent_w, audio_t, **kwargs)


def _patched_init(self, text_len, latent_t, latent_h, latent_w, audio_t,
                  keyframes=None, refs=None, frame_count=None):
    _call_orig_init(self, text_len, latent_t, latent_h, latent_w, audio_t,
...
def _self_test():
    ...
    def build(keyframes=None, refs=None, fix=False, move=False):
        ...
        _call_orig_init(lay, text_len, latent_t, lh, lw, audio_t,
    ...
    stock_kf = [{"resolved_frame_index": 0, "latent": video_latent},
    ...
    run = [{"resolved_frame_index": 0, MC_KEY: i,
            "latent": video_latent} for i in range(4)]
...
def apply_patch():
    global _orig_init, _orig_accepts_frame_count, _applied
    if _applied:
        return True
    ...
    _orig_accepts_frame_count = (
        "frame_count" in inspect.signature(_orig_init).parameters)
    try:
        _self_test()
