...
Use **Generate previews** with matching H3 VAEs to decode stored-frame thumbnails and playable audio. Checkboxes omit frames or voice; drag or use Up/Down to reorder frames. Paired standalone files update their original visual and audio files; Save as a copy creates a combined bundle.
